...
/* Compiler parameter to select the method for determining the dt until the next button event.
    0: Normal interval
...
/* Button is pressed and released at dts sampled from approximated normal distribution with expected value mu and standard deviation sd.
...
/* Button is pressed and released at dts sampled from bernoulli distribution.
...
/* Button is pressed and released at dts sampled from uniform distribution between dt_min and dt_max. */
double random_interval_uniform(double dt_min, double dt_max);

/* Presses and releases the button every dt ms. */
double constant_interval(double dt);

/* Presses and releases the button at intervals defined by sequence of dts. */
double looped_interval(double *dts, int n);

/* Calculates time until next button event based on METHOD parameter. */
