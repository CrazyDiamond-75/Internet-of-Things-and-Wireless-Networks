...
// Store whether the connection is ready for write requests, set after discovery.
static bool conn_ready = false;

// Stores the GATT characteristic handle, filled during discovery.
static uint16_t char_handle;

// Service and characteristic UUIDs, used to check if scanned devices are relevant and to discover the characteristic handle after connecting.
...
static void send_message(message_t *msg)
{
    ...
    // Todo: Maybe copy message to static variable if the asynchronous write request gets overwritten before sending?
...
static int discover_characteristic(struct bt_conn *conn)
{
    ...
    disc_params.start_handle = BT_ATT_FIRST_ATTTRIBUTE_HANDLE;
    disc_params.end_handle = BT_ATT_LAST_ATTTRIBUTE_HANDLE;
...
static void connected(struct bt_conn *conn, uint8_t err)
{
    ...
    // Else, connection succeeded, reset parameters (if they're not resetted) and save the connection reference.
    conn_ready = false;
                         ...
                         uint8_t type, struct net_buf_simple *ad_foreign)
{
    ...
    // Check if the advertised device has the right service UUID in its advertising data and that we're not connected.
    ...
    {
        // If the service doesn't have the right UUID, but we're already connected to the central, redirect the relevant data to the central.
